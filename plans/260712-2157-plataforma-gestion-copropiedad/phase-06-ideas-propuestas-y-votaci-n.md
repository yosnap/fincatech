...
### Addendum (Fase 8): bug de manejo de error `23505` corregido retroactivamente
Durante la revisión de Fase 8 se descubrió, y se verificó empíricamente contra la BD real, que drizzle-orm 0.45.2 (driver `node-postgres`) envuelve los errores de Postgres en un `DrizzleQueryError` cuyo `.code` no existe — el código real queda en `.cause.code`. El check `error.code === '23505'` en `castVote` (línea 104 de esta nota) nunca se disparaba en la práctica: un voto duplicado devolvía un 500 crudo en vez del 409 "Ya emitiste tu voto en esta propuesta" documentado arriba. Corregido junto con Fase 8 usando un helper compartido `getPgErrorCode(error)` (`server/utils/pg-error.ts`). Ver nota de implementación de Fase 8 para el detalle y la verificación.
