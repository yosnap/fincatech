import { z } from 'zod'
import { getExportRows, rowsToPdf } from '../../services/export-service'
...
export default defineEventHandler(async (event) => {
  ...
  const pdf = await rowsToPdf(rows, { start: parsed.data.start, end: parsed.data.end })

  setHeader(event, 'Content-Type', 'application/pdf')
  setHeader(event, 'Content-Disposition', `attachment; filename="gastos_${parsed.data.start}_${parsed.data.end}.pdf"`)
  return pdf
})
