...
// Solo el deudor marca su propia cuota como pagada; comprobante obligatorio (PRD §4.2).
export default defineEventHandler(async (event) => {
  ...
  if (!file?.type || !ALLOWED_TYPES.has(file.type)) {
    throw createError({ statusCode: 400, statusMessage: 'Comprobante requerido (JPEG, PNG o PDF)' })
  }
  ...
  const updated = await markDebtPaid({
    debtId,
    actorId: actor.id,
